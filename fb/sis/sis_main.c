...
sisfb_search_mode(char *name, BOOLEAN quiet)
{
    ...
    //5-6
    //printf("sisfb_search_mode: sisfb_mode_idx:%d\n",sisfb_mode_idx);
...
sisfb_find_rom(struct pci_dev *pdev)
{
	 ...
	 */
    //printf("sisfb_find_rom: before ivideo->nbrigde.\n");
	if(!ivideo->nbridge) {

		if((rom_base = pci_map_rom(pdev, &romsize))) {
            //printf("sisfb_find_rom: after pci_map_rom().\n");
			if(sisfb_check_rom(rom_base, ivideo)) {
                //printf("sisfb_find_rom: after sisfb_check_rom().\n");
    ...
    //printf("sisfb_find_rom: after ivideo->nbrigde.\n");
...
#else
    //printf("sisfb_find_rom: after i386.\n");
	pci_read_config_dword(pdev, PCI_ROM_ADDRESS, &temp);
	pci_write_config_dword(pdev, PCI_ROM_ADDRESS,
			(ivideo->video_base & PCI_ROM_ADDRESS_MASK) | PCI_ROM_ADDRESS_ENABLE);
    //printf("sisfb_find_rom: after PCI read and write config.\n");
	//rom_base = ioremap(ivideo->video_base, 65536);
	rom_base = 0; /*VBIOS is not really needed here */
    //printf("sisfb_find_rom: after PCI read and write config.\n");
	if(rom_base) {
    //printf("sisfb_find_rom: Before sisfb_check_rom.\n");
		if(sisfb_check_rom(rom_base, ivideo)) {
            //printf("sisfb_find_rom: after sisfb_check_rom.\n");
			if((myrombase = vmalloc(65536)))
				memcpy_fromio(myrombase, rom_base, 65536);
...
{
    struct fb_fillrect sisrect = {.dx=dst_x,.dy=dst_y,.width=dim_x,.height=dim_y,.color=color};
    sisfb->fbops->fb_fillrect(sisfb,&sisrect);
}
...
#if 0
	ivideo->video_base = 0xe0000000;
#else
	ivideo->video_base = 0x40000000;
#endif
...
printf(" ivideo->SiS_Pr.SiS_P3c4=%x\n", ivideo->SiS_Pr.SiS_P3c4);
	outSISIDXREG(SISSR, 0x05, 0x86);
//printf(" after	outSISIDXREG(SISSR, 0x05, 0x86).\n");
	if( (!ivideo->sisvga_enabled)
#if !defined(__i386__) && !defined(__x86_64__)
			      || (sisfb_resetcard)
#endif
...
printf(" defined(__i386__) && !defined(__x86_64__)\n");
...
//printf(" after sis.\n");
	...
	if(ivideo->sisfb_userom) {
        //printf("sisfb_probe: before calling sisfb_find_rom().\n");

		ivideo->SiS_Pr.VirtualRomBase = sisfb_find_rom(pdev);
        //printf("sisfb_probe: after calling sisfb_find_rom().\n");
...
error_4:
    ...
    //5-6
    //printf("sisfb_probe: before ivideo->sisfb_mode_idx < 0.\n");

	if((ivideo->sisfb_mode_idx < 0) ||
	   ((sisbios_mode[ivideo->sisfb_mode_idx].mode_no[ivideo->mni]) != 0xFF)) {

        //5-6
        //printf("sisfb_probe: after ivideo->sisfb_mode_idx < 0.\n");
    ...
    andSISIDXREG(SISPART1, 0x2c, 0xc7);
    outSISIDXREG(SISPART1, 0x2e, 0x80);
    andSISIDXREG(SISPART1, 0x1a, 0x3f);
    outSISIDXREG(SISPART1, 0x1f, 0x00);
    outSISIDXREG(SISPART1, 0x20, 0x00);
    outSISIDXREG(SISPART1, 0x21, 0x00);
    outSISIDXREG(SISPART1, 0x22, 0x00);
    outSISIDXREG(SISPART1, 0x23, 0x00);
    outSISIDXREG(SISPART1, 0x16, 0x00);
    outSISIDXREG(SISPART1, 0x17, 0xaa);
    outSISIDXREG(SISPART1, 0x14, 0xb0);
    outSISIDXREG(SISPART1, 0x15, 0x14);
    andSISIDXREG(SISPART1, 0x1d, 0xf8);
    outSISIDXREG(SISPART1, 0x1b, 0x00);
    setSISIDXREG(SISPART1, 0x1d, 0xc7, 0x10);
    outSISIDXREG(SISPART1, 0x1c, 0xff);
    setSISIDXREG(SISPART1, 0x1a, 0xc7, 0x18);
...
static char		*mode = "1360x768-16@60";
static int		vesa = -1;
...
//5-11
int Sis_GetXRes(void)
{
	return sisbios_mode[sisfb_mode_idx].xres;
}

int Sis_GetYRes(void)
{
	return sisbios_mode[sisfb_mode_idx].yres;
}

int Sis_GetBytesPP(void)
{
	return (sisbios_mode[sisfb_mode_idx].bpp+1)/8;
}
...
//#define debug_sis
#ifdef debug_sis
int cmd_insisreg(ac,av)    
    int             ac;
    char           *av[];
{

    unsigned char tmp;
	struct sis_video_info	*ivideo = card_list;
    if (ivideo != NULL)
    {
        ...
        {
            case 'c':
                inSISIDXREG(SISCR,( av[2][0]-'0'),tmp);
                break;
            case 's':
                inSISIDXREG(SISSR,( av[2][0]-'0'),tmp);
                break;
            case 'p':
                SiS_SetRegAND(SISPART1, 0x1a, 0xbf);
                break;
            default:
                inSISIDXREG(SISCR,0,tmp);
                break;
        }
