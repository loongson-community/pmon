...
void setPower(unsigned int nGates, unsigned int Clock);
...
{0x00080200,0x00013005},
	...
	{0x00004, 0x0},
//	{0x00010, 0x07f157c0},
	{0x00048, 0x0002180f}, /*Power Mode1 Gate*/
	{0x0004c, 0xc8090201}, /*Power Mode1 clock*/
	{0x00054, 0x00000000}, /*Power mode ctrl*/
#if defined(CONFIG_VIDEO_8BPP)
	{0x80000, 0x0f014105}, /*Panel display ctrl*/
#endif
#if defined(CONFIG_VIDEO_16BPP)
	{0x80000, 0x0f014105},
#endif
	...
	{0x80010, 0x08000800}, /*panel fb width*/
	{0x80014, 0x04000000}, /*panel window width*/
	{0x80018, 0x02580000}, /*panel window height*/
	{0x8001c, 0x00000000}, /*panel tl*/
	{0x80020, 0x025703ff}, /*panel br*/
	{0x80024, 0x04fa03ff}, /*horizontal total*/
	{0x80028, 0x006c0413},  /*horizontal sync*/
	{0x8002c, 0x02730257}, /*vertical total*/
	{0x80030, 0x00040258}, /*vertical sync*/
#if defined(CONFIG_VIDEO_8BPP)
	...
	{0x00054, 0x00000000},
	{0x00074, 0x0002bcfd},
	{0x00044, 0xc0090201}, /*bit 3-0 dram clock (1: 112M)*/
	{0x0004c, 0xc0090208},
	{0x00040, 0x0002184f},
	{0x80024, 0x053f03ff},
	{0x80028, 0x00880417},
...
#define write8(ptrReg,value) \
    *(volatile unsigned char *)PTR_PAD(sm502.isaBase + ptrReg) = value

#define read16(ptrReg) \
    (*(volatile unsigned short *)PTR_PAD(sm502.isaBase + ptrReg))

#define write16(ptrReg,value) \
    (*(volatile unsigned short *)PTR_PAD(sm502.isaBase + ptrReg) = value)

#define read32(ptrReg) \
    (*(volatile unsigned int *)PTR_PAD(sm502.isaBase + ptrReg))

#define write32(ptrReg, value) \
    (*(volatile unsigned int *)PTR_PAD(sm502.isaBase + ptrReg) = value)

#define regWrite32(ptrReg, value)  (*(volatile unsigned int *)PTR_PAD(sm502.isaBase + 0x100000 + ptrReg) = value)

#define SMTC_write2Dreg(ptrReg, value) (*(volatile unsigned int *)PTR_PAD(sm502.isaBase + 0x100000 + ptrReg) = value)

#define smi_mmiowl_1(dat,reg) (*(volatile unsigned int *)PTR_PAD(sm502.isaBase + reg) = dat)
...
int video_hw_init (void)
{
	...
	printf("video_hw_init after SmiSetRegs().\n");
