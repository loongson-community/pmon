...
s32 ngbe_init_thermal_sensor_thresh(struct ngbe_hw *hw)
{
	...
	wr32(hw, NGBE_TS_INT_EN, NGBE_TS_INT_EN_DALARM_INT_EN);

	wr32(hw, NGBE_TS_EN, NGBE_TS_EN_ENA);


	data->sensor.alarm_thresh = 100;
	wr32(hw, NGBE_TS_ALARM_THRE, 308);/* magic num */
	data->sensor.dalarm_thresh = 90;
	wr32(hw, NGBE_TS_DALARM_THRE, 0x2E0);/* magic num */
				      ...
				      bool *autoneg)
{
	...
	if (hw->device_id == NGBE_DEV_ID_EM_RGMII) {
	...
	if (hw->device_id == NGBE_DEV_ID_EM_WX1860A2S){
		*speed = NGBE_LINK_SPEED_1GB_FULL |
			NGBE_LINK_SPEED_100_FULL |
			NGBE_LINK_SPEED_10_FULL;
		*autoneg = false;
									...
									bool link_up_wait_to_complete)
{
	...
	if (*link_up) {
		if ((value & 0xC000) == 0x0200) {
			*speed = NGBE_LINK_SPEED_1GB_FULL;
		} else if ((value & 0xC000) == 0x0100) {
			*speed = NGBE_LINK_SPEED_100_FULL;
		} else if ((value & 0xC000) == 0x0000) {
			*speed = NGBE_LINK_SPEED_10_FULL;
		}
