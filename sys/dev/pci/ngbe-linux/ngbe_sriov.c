...
static int ngbe_vf_backup(struct ngbe_adapter *adapter, u16 vf)
{
	...
	for (i = 0; i < 16; i++) {
		vfinfo->b4_buf[i] =
			ngbe_rd32(NGBE_BA4_ADDR(vfinfo, i * 4));
	}

	return 0;
}

static int ngbe_vf_restore(struct ngbe_adapter *adapter, u16 vf)
{
	struct vf_data_storage *vfinfo = &adapter->vfinfo[vf];
	int i;
	...
	for (i = 0; i < 16; i++) {
		ngbe_wr32(NGBE_BA4_ADDR(vfinfo, i * 4),
			   vfinfo->b4_buf[i]);
	}
			       ...
			       u32 *msgbuf, u32 vf)
{
	...
	msgbuf[NGBE_VF_TX_QUEUES] = __ALIGN_MASK(1, ~vmdq->mask);
	msgbuf[NGBE_VF_RX_QUEUES] = __ALIGN_MASK(1, ~vmdq->mask);
...
static int ngbe_rcv_msg_from_vf(struct ngbe_adapter *adapter, u16 vf)
{
	...
	case NGBE_VF_BACKUP:
#ifdef CONFIG_PCI_IOV
		retval = ngbe_vf_backup(adapter, vf);
#endif
		break;
	default:
...
static int ngbe_pci_sriov_enable(struct pci_dev __maybe_unused *dev,
