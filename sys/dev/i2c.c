...
again:
	for(j=0;j<2;j++)
	{
		  // choose which i2c device to be used according baseaddr 
		  * GS_SOC_I2C_TXR = addr[j];
		  //* GS_SOC_I2C_CR  = (j == 0)? (CR_START|CR_READ):CR_READ; 
		  * GS_SOC_I2C_CR  = (j == 0)? (CR_START|CR_WRITE):CR_WRITE; 
		   while(*GS_SOC_I2C_SR & SR_TIP);

		  if((* GS_SOC_I2C_SR) & SR_NOACK) { printf("j= %d ,read no ack %d\n",j,__LINE__); i2c_stop();goto again;}
	}
*/
	for(j=0;j<32;j++)
	{
...
int cmd_i2cread(int ac, char *av[])
{
	...
	printf("len = %016u\n", len);
	len = 6;
	//i2c_rec_s(addr,addrlen,buf,count);
	printf("Before i2c read DIMM \n");
	for ( i = 0; i < len; i++)
		printf("%08x\t", buf[i]);
	printf("\n");
	...
	tgt_i2cinit();
	memset(buf,-1,16);
	//i2c_rec_s(addr,len,buf,1);
	i2c_rec_s(addr,len,buf,1);

	printf("After i2c read DIMM \n");
	for ( i = 0; i < len; i++)
		printf("%08x\t", buf[i]);
	printf("\n");
