...
typedef struct {
	volatile unsigned long link;     /* next td/qh (LE)*/
	volatile unsigned long status;   /* status of the td */
	volatile unsigned long info;     /* Max Lenght / Endpoint / device address and PID */
	volatile unsigned long buffer;   /* pointer to data buffer (LE) */

	unsigned long dev_ptr;  /* pointer to the assigned device (BE) */
	unsigned long *data;    //
	unsigned long pipe;		//mainly for control unaligned transfer 
	unsigned long res[1];   /* reserved (TDs must be 8Byte aligned) */
...
struct uhci {
	...
	struct portstatus *portinfo;
	unsigned int numports;
	unsigned long devmap[128/(8*sizeof(unsigned long))];
