...
//	printf("addr=%d,len=%d",addr,len);
retry:
//	puts("before mtd->read!!");
...
//		puts("before block_isbad!!");
//		printf("mtd=%p,mtd->priv=%p\n",mtd,mtd->priv);
//		printf("pnum=%d,ubi->peb_size=%d\n",pnum,ubi->peb_size);
		ret = mtd->block_isbad(mtd, (loff_t)pnum * ubi->peb_size);
//		puts("after block_isbad!!");
		if (ret < 0)
			ubi_err("error %d while checking if PEB %d is bad",
				ret, pnum);
		else if (ret)
			dbg_io("PEB %d is bad", pnum);
//		printf("peb %d is good!!",pnum);
		return ret;
	}
