...
void vga_roll()
{
	...
	for (i = 0; i < (rowcount - 1) * colcount; i++)
		*(unsigned char *)(vgabh + 2 * i) =
		    *(unsigned char *)(vgabh + 2 * (i + colcount));
...
int vgaterm(int op, struct DevEntry *dev, unsigned long param, int data)
{
	unsigned char code;
	switch (op) {
	case OP_INIT:
...
int fbterm(int op, struct DevEntry *dev, unsigned long param, int data)
{
