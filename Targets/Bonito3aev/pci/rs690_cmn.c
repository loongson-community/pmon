 ...
 *  2010-08-12    QianYuli        PMON20100812    Move typedef unsigned 
 *                                               long long u64 to sys/linux/types.h
 ************************************************************************************************/
...
u32 _pci_conf_readn(device_t tag, int reg, int width)
{
	...
	if(width==1) return _pci_conf_read8(tag,reg); //zxj
	else if(width==2) return _pci_conf_read16(tag,reg); //zxj
