...
#ifndef LOONGSON_3C2H

#define	AT24C64_ADDR	0xa0
#define LS2H_I2C_REG_BASE           LS2H_I2C1_REG_BASE           
#define LS2H_I2C_PRER_LO_REG        LS2H_I2C1_PRER_LO_REG        
#define LS2H_I2C_PRER_HI_REG        LS2H_I2C1_PRER_HI_REG        
#define LS2H_I2C_CTR_REG            LS2H_I2C1_CTR_REG            
#define LS2H_I2C_TXR_REG            LS2H_I2C1_TXR_REG            
#define LS2H_I2C_RXR_REG            LS2H_I2C1_RXR_REG            
#define LS2H_I2C_CR_REG             LS2H_I2C1_CR_REG             
#define LS2H_I2C_SR_REG             LS2H_I2C1_SR_REG             

#else

#define	AT24C64_ADDR	0xa8
#define LS2H_I2C_REG_BASE           LS2H_I2C0_REG_BASE           
#define LS2H_I2C_PRER_LO_REG        LS2H_I2C0_PRER_LO_REG        
#define LS2H_I2C_PRER_HI_REG        LS2H_I2C0_PRER_HI_REG        
#define LS2H_I2C_CTR_REG            LS2H_I2C0_CTR_REG            
#define LS2H_I2C_TXR_REG            LS2H_I2C0_TXR_REG            
#define LS2H_I2C_RXR_REG            LS2H_I2C0_RXR_REG            
#define LS2H_I2C_CR_REG             LS2H_I2C0_CR_REG             
#define LS2H_I2C_SR_REG             LS2H_I2C0_SR_REG             
...
{
again:
	write_reg_byte(LS2H_I2C_CR_REG, CR_STOP);
	read_reg_byte(LS2H_I2C_SR_REG);
	while (read_reg_byte(LS2H_I2C_SR_REG) & SR_BUSY)
		goto again; 
}
void ls2h_i2c1_init()
{
	write_reg_byte(LS2H_I2C_CTR_REG, 0x0);
	write_reg_byte(LS2H_I2C_PRER_LO_REG, 0xc8);
	write_reg_byte(LS2H_I2C_PRER_HI_REG, 0x0);
	write_reg_byte(LS2H_I2C_CTR_REG, 0x80);
}
...
static int i2c_send_addr(int data_addr)
{
	...
	write_reg_byte(LS2H_I2C_TXR_REG, ee_dev_addr);
	write_reg_byte(LS2H_I2C_CR_REG, (CR_START | CR_WRITE));
	while (read_reg_byte(LS2H_I2C_SR_REG) & SR_TIP) ;

	if (read_reg_byte(LS2H_I2C_SR_REG) & SR_NOACK) {
		printf("%s:%d  eeprom has no ack\n", __func__, __LINE__);
		ls2h_i2c_stop();
		return 0;
	}

	write_reg_byte(LS2H_I2C_TXR_REG, (i & 0xff));
	write_reg_byte(LS2H_I2C_CR_REG, CR_WRITE);
	while (read_reg_byte(LS2H_I2C_SR_REG) & SR_TIP) ;
	...
	write_reg_byte(LS2H_I2C_TXR_REG, (data_addr & 0xff));
	write_reg_byte(LS2H_I2C_CR_REG, CR_WRITE);
	while (read_reg_byte(LS2H_I2C_SR_REG) & SR_TIP) ;
...
int eeprom_write_byte(int data_addr, unsigned char *buf)
{
	...
	write_reg_byte(LS2H_I2C_TXR_REG, (*buf & 0xff));
	write_reg_byte(LS2H_I2C_CR_REG, CR_WRITE);
	while (read_reg_byte(LS2H_I2C_SR_REG) & SR_TIP) ;
...
int eeprom_write_page(int data_addr, unsigned char *buf, int count)
{
	...
	for (i = 0; i < count; i++) {
		write_reg_byte(LS2H_I2C_TXR_REG, (buf[i] & 0xff));
		write_reg_byte(LS2H_I2C_CR_REG, CR_WRITE);
		while (read_reg_byte(LS2H_I2C_SR_REG) & SR_TIP);
...
int eeprom_read_cur(unsigned char *buf)
{
	...
	write_reg_byte(LS2H_I2C_CR_REG, CR_READ);
	while (read_reg_byte(LS2H_I2C_SR_REG) & SR_TIP) ;

	*buf = read_reg_byte(LS2H_I2C_RXR_REG);
	ls2h_i2c_stop();
...
static int i2c_read_seq_cur(unsigned char *buf, int count)
{
	...
	for (i = 0; i < count; i++) {
		write_reg_byte(LS2H_I2C_CR_REG, 
				((i == count - 1) ? (CR_READ | CR_ACK) : CR_READ));
		while (read_reg_byte(LS2H_I2C_SR_REG) & SR_TIP) ;
		buf[i] = read_reg_byte(LS2H_I2C_RXR_REG);
	}
	ls2h_i2c_stop();
	return i;
}
...
static const Cmd Cmds[] = {
	{"Misc"},
	{"i2c1init", "", NULL, "init LS2H I2C", ls2h_i2c1_init, 1, 5, 0},
